...
export const contactApiFunctions = {
  submitContactForm: async (data: ContactFormData): Promise<{ message: string; success: boolean }> => {
    const response = await contactApi.post('/submit', data);
    return response.data;
  },

  getContactSubmissions: async (page: number = 1, limit: number = 10, search: string = ''): Promise<ContactSubmissionsResponse> => {
...
export const useContactSubmissions = (page: number = 1, limit: number = 10, search: string = '') => {
  return useQuery({
    queryKey: ['contactSubmissions', page, limit, search],
    queryFn: () => contactApiFunctions.getContactSubmissions(page, limit, search),
