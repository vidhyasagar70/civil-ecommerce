...
export const useAdminThemeStyles = () => {
    ...
    const cardStyle = (variant: 'primary' | 'secondary' = 'secondary'): CSSProperties => ({
        backgroundColor: variant === 'primary' ? colors.background.primary : colors.background.secondary,
    ...
    const buttonStyle = (variant: 'primary' | 'secondary' | 'danger' = 'primary'): CSSProperties => {
    ...
    const statusBadgeStyle = (status: 'success' | 'warning' | 'error' | 'info'): CSSProperties => ({
...
export const getThemeAwareClassName = (baseClass: string, theme: 'light' | 'dark'): string => {
