...
interface Banner {
  ...
  bannerType: "festival" | "flashsale" | "seasonal" | "normal";
...
const BannerCarousel: React.FC<BannerCarouselProps> = ({ page }) => {
  ...
  if (isClosed || banners.length === 0) return null;
        ...
        <div key={banner._id} className="relative z-40 w-full max-w-3xl px-4 sm:px-8 text-center flex flex-col items-center gap-3 animate-fade-in">
          ...
          <div className="flex flex-wrap justify-center gap-4 mt-4">
            {banner.ctaButtonText && (
              <button onClick={() => handleClick(banner.ctaButtonLink)} className="px-4 py-3 relative rounded-2xl font-bold bg-white text-black hover:scale-105 hover:shadow-lg transition-transform duration-300 shadow-xl">
                {banner.ctaButtonText}
              </button>
            )}
            {banner.secondaryButtonText && (
              <button onClick={() => handleClick(banner.secondaryButtonLink)} className="px-4 py-3 rounded-full font-semibold bg-transparent text-white border border-white/50 hover:bg-white/10 transition-all duration-300">
                {banner.secondaryButtonText}
              </button>
            )}
          </div>
          ...
          <div className="absolute bottom-4 left-0 right-0 flex justify-center space-x-2 z-30">
            {banners.map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all duration-300 ${index === current ? "bg-white scale-125 shadow-md" : "bg-white/50 hover:bg-white/70"}`}
              />
            ))}
          </div>
        )}
